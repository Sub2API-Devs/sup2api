export default {
    accounts: {
      ...
      accountLocation: '账号存储位置',
      mainServerAccounts: '主服务器账号',
      workerAccounts: 'Worker 账号',
      workerLabel: '归属 Worker ·',
      searchWorkerAccounts: '搜索 Worker 账号...',
      allWorkers: '全部 Worker',
      allWorkersCount: '全部 Worker（{count}）',
      workerFilter: '按 Worker 筛选账号',
      workerAccountCount: '{count} 个账号',
      workerCapacityManaged: 'Worker 管理',
      workerSchedulingHint: '该账号的调度状态由所属 Worker 管理',
      workerUsageUnavailable: 'Worker 暂未上报用量窗口',
      lastTest: '最近测试',
      editWorkerAccount: '编辑 Worker 账号',
      workerTestModel: '测试模型',
      workerEditCredentialHint: '编辑只更新账号元数据，不会读取或覆盖 Worker 内保存的凭据。',
      workerAccountUpdated: 'Worker 账号已更新',
      workerAccountUpdateFailed: '更新 Worker 账号失败',
      workerPartialLoadFailed: '有 {count} 个 Worker 暂时无法同步，当前显示其余 Worker 的账号。',
      noWorkerAccounts: '暂无 Worker 账号',
      noWorkerAccountsHint: '点击“添加账号”并选择目标 Worker，账号创建后仍会在此处统一管理。',
      allModels: '全部模型',
      refreshWorkerCredential: '刷新 Worker 内的账号凭据',
      testWorkerAccount: '在 Worker 内测试账号',
      test: '测试',
      deleteWorkerAccount: '删除 Worker 账号',
      deleteWorkerAccountConfirm: '确定删除 Worker 账号“{name}”吗？该操作会永久删除 Worker 内保存的真实凭据。',
      workerAccountsLoadFailed: '加载 Worker 账号失败',
      workerCredentialRefreshed: 'Worker 账号凭据刷新成功',
      workerCredentialRefreshFailed: '刷新 Worker 账号凭据失败',
      workerAccountTestPassed: 'Worker 账号测试通过',
      workerAccountTestFailed: 'Worker 账号测试失败',
      workerTestOwner: '归属 {name}',
      workerTestOwnerLine: '归属 Worker：{worker}',
      workerTestModelUnavailable: '该 Worker 账号尚未配置可测试模型，请先在编辑中填写模型。',
      workerTestHttpStatus: 'HTTP 状态：{status}',
      workerTestLatency: '响应耗时：{latency} ms',
      workerTestEmptyResponse: '上游返回了空响应正文',
      workerAccountDeleted: 'Worker 账号已删除',
      workerAccountDeleteFailed: '删除 Worker 账号失败',
      createAccount: '添加账号',
      autoRefresh: '自动刷新',
      ...
      openai: {
        ...
        codexFingerprintThreadPool: '设备+会话+线程池（推荐）',
      ...
      accountCreatedOnWorker: '账号已添加到 Worker',
      addToWorkerHint: '选中后账号凭据只保存在该 Worker，不会写入主库。当前支持 API Key 账号；OAuth 请在 Worker 管理页完成。',
      workerAccountUnsupported: '该账号类型暂不支持直接写入 Worker，请改用 Worker 管理页或取消 Worker 目标。',
      workerAccountKeyRequired: '写入 Worker 需要 API Key',
      workerOAuthUnsupported: 'OAuth 账号请在 Worker 管理页添加，或取消“添加到 Worker”后在本机创建。',
