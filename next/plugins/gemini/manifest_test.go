...
func TestManifest(t *testing.T) {
	...
	if len(at.GuardedSettings) != 1 || at.GuardedSettings[0].Field != "base_url" ||
		!slices.Equal(at.GuardedSettings[0].Allowed, []string{gemini.DefaultBaseURL}) {
		t.Fatalf("guardedSettings = %+v, want base_url -> [%s]", at.GuardedSettings, gemini.DefaultBaseURL)
	}
	var schema struct {
