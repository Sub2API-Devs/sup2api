...
func buildCompactRequest(root map[string]any) error {
	input, err := normalizeCompactInput(root["input"])
	if err != nil {
		return err
	}
	input = append(input, map[string]any{
		"type": "message", "role": "user",
		"content": []any{map[string]any{"type": "input_text", "text": compactSummaryPrompt}},
	})
	root["input"] = input
...
func normalizeCompactInput(value any) ([]any, error) {
...
func convertCompactInputs(root map[string]any) {
	...
	for _, raw := range items {
		...
		if !ok || (stringValue(item["type"]) != "compaction" && stringValue(item["type"]) != "compaction_summary") {
	...
	if changed {
		root["input"] = converted
	}
}

func convertCompactResponse(payload []byte) ([]byte, error) {
	response, err := decodeJSONObject(payload)
	if err != nil {
		return nil, err
	}
	...
	item := map[string]any{
		"id": "cmp_" + randomHexID(), "type": "compaction", "status": "completed", "encrypted_content": encrypted,
	}
